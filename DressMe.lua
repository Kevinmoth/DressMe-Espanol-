...
local mainFrameTitle = "|ccff6ff98DressMe |cffffffff(Traducido por Matecitos)|r"
...
do
    ...
    tip:SetText("\124cff00ff00click izquierdo:\124r Girar \124 \124cff00ff00Click Derecho:\124r Mover\124n\124cff00ff00Rueda\124r o \124cff00ff00Alt + Click Derecho:\124r Zoom")
...
do
    StaticPopupDialogs["DRESSME_SEND_DIALOG"] = {
        text = "|cffffd700Nombre del personaje:",
        button1 = "Enviar",
        button2 = CLOSE,
        timeout = 0,
        whileDead = true,
        ...
        OnAccept = function(self)
            ...
            if playerName ~= "" then
                ...
                if itemsCount > 0 then
                    ...
                    SELECTED_CHAT_FRAME:AddMessage("|ccff6ff98<DressMe>|r: No hay nada que enviar.")
                end
            end
        end,
...
do
    ...
    btn:HookScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Usar el modelo del target")
        GameTooltip:AddLine("El target debe estar en rango", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
...
local TAB_NAMES = {"Probador", "Mis Tranfis", "Configuraciones"}
...
local function slot_OnShiftLeftClick(self)
    if self.itemId ~= nil then
        ...
        if link ~= nil then
            ...
            SELECTED_CHAT_FRAME:AddMessage("|ccff6ff98<DressMe>|r: Parece que este objeto no se puede utilizar para la transfi.")
        end
    end
end
...
local function slot_OnEnter(self)
    ...
    if self.itemId == nil then
        ...
        if GetSettings().showShortcutsInTooltip then
            GameTooltip:AddLine(" ")
            GameTooltip:AddLine("|cff00ff00Shift + Left Click:|r Crear un link del objeto.")
            GameTooltip:AddLine("|cff00ff00Right Click:|r Borrar item de la ranura.")
            GameTooltip:AddLine("|cff00ff00Ctrl + Left Click:|r Crear Link a Wowhead.")
        end
    end
    GameTooltip:Show()
end
...
do
    ...
    btn:SetScript("OnEnter", function(self)
        GameTooltip:ClearLines()
        GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT")
        GameTooltip:AddLine("FormaDeSombras")
        GameTooltip:AddLine("Una simulación pobre que depende de la configuración de luz del modelo. El equipo con su propia fuente de luz (como el equipo Excavator's Brand) da un resultado erróneo. Las texturas de emisión que ignoran la luz (como los ojos Draenei) también ignoran la simulación y no están sombreadas como lo estarían en la verdadera forma de sombra..", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
...
do
    ...
    slider:SetScript("OnMinMaxChanged", function(self, min, max)
        label:SetText(("Pagina: %s/%s"):format(self:GetValue(), max))
    end)
    ...
    slider:SetScript("OnValueChanged", function (self, value)
        ...
        label:SetText(("Pagina: %s/%s"):format(value, max))
    end)
...
do
    ...
    list.onEnter = function(self)
        ...
        GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT")
        GameTooltip:ClearLines()
        GameTooltip:AddLine("Esta apariencia se puede obtener de el/los siguientes objetos:", 1, 1, 1)
        GameTooltip:AddLine(" ")
        ...
        if GetSettings().showShortcutsInTooltip then
            GameTooltip:AddLine("|n|cff00ff00Click Derecho:|r probar la trasfi.")
            if #ids > 1 then
                GameTooltip:AddLine("|cff00ff00Tab:|r elija un elemento de la lista.")
                GameTooltip:AddLine("|cff00ff00Shift + Left Click:|r crear un hlink para el elemento elegido.")
            else
                GameTooltip:AddLine("|cff00ff00Shift + Left Click:|r crear un hlink para el elemento elegido.")
            end
            GameTooltip:AddLine("|cff00ff00Ctrl + Left Click:|r Crea un link a WowHead")
        end
        GameTooltip:Show()
...
do
    ...
    btnTryOn:HookScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Puede no ser inmediato si hay elementos en el aspecto elegido que deben ser consultados y almacenados en cache.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
...
do
    ...
    label:SetJustifyH("CENTER")
    label:SetHeight(10)
    label:SetText("Apariencias recibidas")
    ...
    listFrame:SetScript("OnEvent", function(self, event, prefix, message, channel, sender)
        if prefix == addonMessagePrefix then
            ...
            if GetSettings().announceAppearanceReceiving then
                SELECTED_CHAT_FRAME:AddMessage("|ccff6ff98<DressMe>|r: Recibiste una trasfiguracion de "..sender..".")
            end
        end
    end)
...
do  --------- email and github
    ...
    email:SetHeight(10)
    email:SetText("Creador:GetLocalPlayer | Modificado y traducido por @Matecitos")
end
...
do  --------- Preview Setup
    ...
    tipFrame:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT")
        GameTooltip:ClearLines()
        GameTooltip:AddLine("Modelos usados")
        GameTooltip:AddLine("Hay una modificación hecha por diversión para el cliente de WotLK que trae modelos de personajes modernos de alta calidad de la expansión Warlords of Draenor (Como NaerZone). Desafortunadamente, la vista previa de los modelos modernos tiene una configuración diferente. Si tu cliente de juego utiliza los modelos modernos, elige moderno en este menú emergente y clásico en caso contrario. ", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
...
do  --------- Character background
    ...
    label:SetText("Fondo del personaje:")
...
do  --------- Show/hide "DressMe" button
    ...
    checkbox:HookScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT")
        GameTooltip:ClearLines()
        GameTooltip:AddLine("Mostrar el boton de \"DressMe\"")
        GameTooltip:AddLine("Mostrar o ocultar el boton de \"DressMe\" el la ventana de personaje.", 1, 1, 1, 1, true)
        GameTooltip:AddLine("tambien puede acceder usando el comando \"/dressme\" en el chat.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
    ...
    label:SetText("Mostrar el boton \"DressMe\".")
...
do  --------- Show shortcuts in tooltips
    ...
    local label = checkbox:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    label:SetText("Mostrar atajos en tooltips.")
    label:SetPoint("LEFT", checkbox, "RIGHT", 4, 2)
end
...
do  --------- Hide hair on cloak preview
    ...
    local label = checkbox:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    label:SetText("Ocultar pelo en la vista previa de la capa")
    label:SetPoint("LEFT", checkbox, "RIGHT", 4, 2)
end
...
do  --------- Hide hair and beard on chest preview
    ...
    local label = checkbox:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    label:SetText("Ocultar pelo y barba en el pecho,\ncamisa, y vistas previas de tabardo")
...
do  --------- Use server time in received appearances
    ...
    local label = checkbox:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    label:SetText("Usar la hora del servidor para las \"apariencias\nrecibidas\"")
    label:SetJustifyH("LEFT")
    label:SetPoint("LEFT", checkbox, "RIGHT", 4, 2)
end
...
do  --------- Announce appearance receiving
    ...
    local label = checkbox:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    label:SetText("Anuncia en el chat si se ha recibido una aparición\nde otro jugador.")
    label:SetJustifyH("LEFT")
    label:SetPoint("LEFT", checkbox, "RIGHT", 4, 2)
end
...
do  --------- Ignore UI sclaing
    ...
    checkbox:HookScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT")
        GameTooltip:ClearLines()
        GameTooltip:AddLine("Ignorar escala de UI.")
        GameTooltip:AddLine("El renderizado 3D del juego puede romper la correcta visualización de las vistas previas con valores demasiado pequeños de escalado de IU en los ajustes de vídeo. Activa esta casilla para ignorar el escalado de la IU.", 1, 1, 1, 1, true)
        GameTooltip:Show()
    end)
    checkbox:HookScript("OnLeave", function(self)
        GameTooltip:Hide()
    ...
    local label = checkbox:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    label:SetText("Ignorar escalado de UI")
    label:SetPoint("LEFT", checkbox, "RIGHT", 4, 2)
end
